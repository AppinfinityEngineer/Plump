// Small dependency-free HMAC-SHA256 signer for staging device writes.
// This protects the staging backend from open public writes. Production should
// replace the public Expo secret with device registration / issued device keys.
...
export function hmacSha256Hex(secret: string, message: string): string {
  ...
  const outer = key.map((b) => b ^ 0x5c);
  const inner = key.map((b) => b ^ 0x36);
  const innerHash = sha256Bytes([...inner, ...utf8Bytes(message)]);

  return bytesToHex(sha256Bytes([...outer, ...innerHash]));
}
